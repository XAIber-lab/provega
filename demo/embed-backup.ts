...
async function _embed(
  ...
  // ── NORMALIZZAZIONE spec E BUFFER DEI PARTIAL RESULTS ──
  // Prepara il buffer per originalValues
  let originalValues: any[] = [];
  let isColorEncoding = false;
  let socketCheck = false;

  if(spec.data.url && /^wss?:\/\//.test(spec.data.url)) {
  ...
  if (spec && spec !== true) {
    ...
    // Nome dataset
    //const datasetName = dataSpec.name || "progressiveData";
    ...
    dataSpec.provega.progression.chunking.type = dataSpec.provega.progression.chunking.type || "data";
    ...
    dataSpec.provega.progression.chunking.reading.ascending = dataSpec.provega.progression.chunking.reading.ascending !== false;
    ...
    console.log(vgSpec)
    // Trova il dataset in vgSpec.data
    const dataset = vgSpec.data.find((d: any) => d.name === 'source_0');

    console.log(vgSpec, vgSpec.data)
    console.log("Dataset found:", dataset);

    // 1) Se i valori sono partialResult wrappers, bufferizzali in _initialPartials
    if (
    ...
    // 2) Otherwise in "data" mode: buffer original values into _initialData
  ...
  // ── INIZIALIZZAZIONE spec.progressive ──
  ...
  const q = spec.provega.progression.monitoring.quality = spec.provega.progression.monitoring.quality || {};
  ['absolute_progress', 'relative_progress', 'relative_stability', 'absolute_certainty']
    .forEach(key => {
      q[key] = q[key] || {};
  ...
  const ch = spec.provega.progression.monitoring.change
 ...
 || {};
  ...
  // Questi salveranno i transform di partenza
  let initX = 0;
  let initY = 0;
  ...
  function highlightChange(chunk: any[], progressID: number) {
    ...
    // Rimuovo eventuali griglie default
    ...
    // —————————————————————————
    // 1) Creo / aggiorno la griglia overlay
    // —————————————————————————
    ...
    // —————————————————————————
    // 2) Se non inizializzato, assegno IDs e monto l'observer
    // —————————————————————————
    if (!observerInitialized) {
        ...
        muts.forEach(mut => {
          if (mut.type === 'attributes' && mut.attributeName === 'transform') {
            ...
            if (tgt.id === 'xTickRef' || tgt.id === 'yTickRef') {
              ...
              // applica un solo translate cumulativo
              ...
              // una volta applicato, aggiorno init e faccio disconnect temporaneo
      ...
      // d) avvio l'osservazione SOLO su quei due elementi
      ...
      setTimeout(() => {
        ...
        newMarks.forEach(m => {
          ...
          if (cfg.mark.blink) {
            ...
            // altrimenti dal fill/stroke correnti
            ...
            // Se non li hai ancora impostati, salvali tu stesso
            if (!m.hasAttribute('data-orig-fill') && m.hasAttribute('fill')) {
            ...
            const iv = setInterval(() => {
              ...
              if (count >= iterations) {
                clearInterval(iv);
                // Assicuriamoci di ripristinare definitivamente
  ...
  if (qualitySignals.length > 0) {
    ...
    Object.assign(panel.style, {
        ...
        boxShadow: '0 0.2em 0.8em rgba(0,0,0,0.08)',
        fontFamily: 'Arial, sans-serif', fontSize: '0.3em'
    });
    ...
    const W2 = 65, MAX_POINTS = 50;
    // ora history contiene array di { t: numero, v: numero }
    const history: Record<string, { t: number, v: number }[]> = {};

    const lll = qualitySignals.length;
    const totalGap = (lll - 1) * 1.3;
    const itemHeight = (100 - totalGap) / lll;

    const H2 = itemHeight - 1.2;
    ...
    qualitySignals.forEach(sig => {
      ...
      chartWrapper.style.width = `${W2 + 2}px`;
      chartWrapper.style.height = `55%`; // + spazio per etichette X
      ...
      // Etichetta per il tempo minimo (primo timestamp) e per il tempo massimo
      ...
      view.addSignalListener(sig, (_: any, v: number) => {
        // aggiorna lo span testuale
        span.textContent = `${label}: ${v != null ? v.toFixed(3) : '—'}`;

        // salva timestamp corrente (millisecondi) e valore (in numero)
        const now = Date.now();
        historyBuf.push({ t: now, v: v == null ? 0 : v });
        if (historyBuf.length > MAX_POINTS) historyBuf.shift();

        // Controllo se tutti i valori sono 0 o null/undefined
        const allZero = historyBuf.every(pt => pt.v === 0);
        if (allZero) {
          // non disegno il grafico, vuoto il polyline e le etichette X
        ...
        const spanT = Math.max(t_max - t_min, 1); // per evitare divisioni per zero

        // Trova valore min e max per scala Y
        const values = historyBuf.map(pt => pt.v);
        const mn = Math.min(...values);
        const mx = Math.max(...values);
        const spanY = Math.max(mx - mn, 1e-6);

        // Calcola punti per il polyline
        const pts = historyBuf.map(pt => {
          const x = ((pt.t - t_min) / spanT) * (W2);
          const y = H2*0.8617 - ((pt.v - mn) / spanY) * H2*0.8617;
          return `${x},${y}`;
        }).join(' ');
  ...
  // ── Helper per applicare gli stili definiti dall’utente ──
  function applyStyle(
  ...
  // Dichiarazione globale per Socket.IO (se usato)
  declare var io: any;
  ...
  (async function progressiveLoadingHandler() {
    ...
    spec = spec || {} as any;
    const dataSpec = spec as any;
    if(dataSpec.name === undefined) dataSpec.name = "source_0";
    ...
    if (chunkType === 'process') {
      ...
      console.log("[Progressive Loading] Process chunking type detected, initializing process handler.");
    }
    

    // ─── 1) Creo o recupero il footer wrapper ───
    ...
    // ─── 2) Creo o recupero progress-container (sinistra) ───
    ...
    if (!progressContainer) {
      ...
      // Imposto larghezza fissa per la progress bar (se desiderato)
    ...
    if (showProcessPanel) {
      ...
      if (!procPanel) {
        ...
        Object.assign(procPanel.style, {
          ...
          boxShadow: 'rgba(0, 0, 0, 0.08) 0px 0.125em 0.5em',
...
// All’inizio, catturiamo i domini “naturali” e calcoliamo il padded domain
if (visRep.visual_stability) {
  view.runAsync().then(() => {
    try {
      ...
      /**
       * Riduce il noise su un chunk:
       *  1) Clipping a ±2σ
       *  2) Moving average su finestra di dimensione windowSize
       *
       * @param chunk      Array di oggetti { x, y, ... }
       * @param windowSize Dimensione finestra per lo smoothing (numero di punti totali)
       */
      function noiseReduce(chunk: any[], windowSize = 5): any[] {
        ...
        // -- 2) Moving average ---------------------------------------------------
        // Costruisco due array di soli valori x e y
        const xsCl = clipped.map(d => d.x);
        const ysCl = clipped.map(d => d.y);
        const halfWin = Math.floor(windowSize / 2);

        // Funzione che calcola la media dei vicini in [i-halfWin…i+halfWin]
        ...
        // Ricompongo gli oggetti con le nuove coordinate “smoothed”
      ...
      async function insertNextChunk(manualStep: boolean = false, force: boolean = false) {
        ...
        // 2) Prepara il chunk: SEQUENZIALE vs RANDOM
        let indices = [];
        if (readingType === "random") {
          // Estrai un set di indici casuali dal rimanente
        ...
        // 3) Estrai il rawChunk e metadata
        ...
        // 4) Segnali di quality in ingresso
        const idx = loaded;
        ...
        if (readingType === "random") {
          // Rimuovi quegli indici da originalValues
  ...
  // Crea i domini finali mantenendo il padding iniziale
  ...
  // Imposta i domini “bloccati”
  view.scale("x").domain(finalX);
  view.scale("y").domain(finalY);

  // Rirenderizza con i nuovi domini
  await view.runAsync();
}
        ...
        // 12) Schedule del prossimo chunk
      ...
      dataSpec._insertNextChunk = insertNextChunk; //esporta la funzione per l'uso esterno
      ...
      // Espongo la funzione per i bottoni
      dataSpec._insertNextChunk = insertNextChunk;

      // Avvio la progressione
      insertNextChunk();
    }
    // ----- Fine progressiveLoading -----
    ...
    lockState = false;
      ...
      if (!isColorEncoding){
        ...
        dataSpec._startBtn.addEventListener("click", () => {
          ...
          if ((dataSpec._auxBuffer?.length || 0) === 0
            ...
            // Ricostruisco il buffer da tutti i chunk rimossi
            const chunksToRedo = dataSpec._undoneChunks
              // opzionale: ordina per chunk.id se vuoi riavere l'ordine originale
              .sort((a, b) => a.id - b.id)
              .flatMap(entry => entry.chunk);

            // Ripopolalo
            dataSpec._auxBuffer = chunksToRedo.slice();
            // Svuota gli undone (li stiamo per reinserire tutti)
            dataSpec._undoneChunks = [];
            console.log(
          ...
          // 2) Imposta lo stato e ricomincia
          if (progressState !== "playing") {
      ...
      // — Event listener per Next Step sul ramo “non‑colors” —
      if (exec.pause && !isColorEncoding) {
        dataSpec._nextStepBtn.addEventListener("click", () => {
          ...
          // 1) reinserimento chunk “undo‑abile”
          ...
          // 2) fallback “manual load”: se ho ancora chunk da caricare
          //    aggiorno prima nextIndex, poi chiamo il metodo
          if (dataSpec._undoneChunks.length === 0 &&
            ...
            // riallinea nextIndex come prima
      ...
      // — Event listener per Step Back (se abilitato) —
      if (exec.backward) {
        dataSpec._stepBackBtn.addEventListener("click", () => {
          if (lockState) return;
          lockState = true;

          // Se eravamo in final, torniamo a paused
          ...
          if (history && history.length > 0) {
            ...
            if (Array.isArray(lastEntry.chunk)) {
              // 1) accumula per il redo
              dataSpec._undoneChunks.push(lastEntry);

              // 2) rimuovi dalla view
              ...
              // 4) disabilita Step Back se siamo tornati al primo
              if (lastEntry.id === 1) {
    ...
    if (socketCheck==true) {
      ...
      // tieni una copia per lo StartOver
      dataSpec._initialData = originalValues.slice();
      // svuota values per non ridisegnare tutto in partenza
      dataSpec.values = [];
  ...
  if (dataSpec.data.url.toLowerCase().endsWith('.csv')) {
    fetch(dataSpec.data.url)
      ...
      .then(text => {
        // 1) parse CSV con Vega (no dipendenza da d3)
        const all = vega.read(text, { type: 'csv' });
        console.log("[Progressive Loading] CSV parsed, colonne trovate:", Object.keys(all[0]||{}));

        // 2) rileva dinamicamente i nomi delle colonne lat/lon
        ...
        // 3) costruisci l’array da caricare
        let toLoad;
        if (latKey && lonKey) {
          // mappa su {lat, lon, ...altri campi}
          toLoad = all.map(d => ({
            ...d,
            lat: +d[latKey],
            lon: +d[lonKey]
          }));
          console.log(`[Progressive Loading] Trovate lat/lon in '${latKey}', '${lonKey}'. Caricati ${toLoad.length} punti.`);
        } else {
          // no lat/lon: load the full CSV as-is
          toLoad = all;
          console.warn("[Progressive Loading] Colonne latitude/longitud non trovate, carico tutti i campi.");
        }

        // 4) reset layer e avvia progressiveLoading
        view.change(datasetName, vega.changeset().remove(() => true)).run();
    ...
    // Branch JSON inalterato
    fetch(dataSpec.data.url)
      .then(response => response.json())
      .then(fetchedData => {
        console.log("[Progressive Loading] JSON fetched (prime 5):", fetchedData.slice(0,5));
        view.change(datasetName, vega.changeset().remove(() => true)).run();
  ...
  })(); // Fine IIFE progressiveLoadingHandler
